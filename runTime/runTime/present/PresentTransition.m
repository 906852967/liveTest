...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext{
    ...
    switch (_transitionType) {
        case TransitionOneTypePresent:
            [self presentAnimation:transitionContext];
            break;
        case TransitionOneTypeDissmiss:
            [self dismissAnimation:transitionContext];
            break;
        case TransitionOneTypePush:
            [self pushAnimation:transitionContext];
            break;
        case TransitionOneTypePop:
...
- (void)presentAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    TestVC * toVC = (TestVC *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    TransformVC * fromVC;
    if ([[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey] isKindOfClass:[UINavigationController class]]) {
        UINavigationController * na = (UINavigationController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
        fromVC = na.viewControllers.lastObject;
    }else if([[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey] isKindOfClass:[TransformVC class]]){
        fromVC = (TransformVC *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    }
...
//    tempView.frame = [cell.imageView convertRect:cell.imageView.bounds toView:containerView];
    ...
    containerView.backgroundColor = [UIColor colorWithWhite:0.8 alpha:0.5];
...
- (void)dismissAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    //通过viewControllerForKey取出转场前后的两个控制器，这里toVC就是转场后的VC、fromVC就是转场前的VC
    TransformVC * toVC;
    TestVC * fromVC = (TestVC *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    if ([[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey] isKindOfClass:[UINavigationController class]]) {
        UINavigationController * na = (UINavigationController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
        toVC = na.viewControllers.firstObject;
    }else if([[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey] isKindOfClass:[TransformVC class]]){
        toVC = (TransformVC *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    }
    //取出转场前后视图控制器上的视图view
    UIView * toView = [transitionContext viewForKey:UITransitionContextToViewKey];
    UIView * fromView = [transitionContext viewForKey:UITransitionContextFromViewKey];
